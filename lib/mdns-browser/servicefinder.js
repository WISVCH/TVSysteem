 ...
 */
var ServiceFinder = function(callback, opt_services) {
  ...
  if(!ServiceFinder.api) return;

  // Store published services
  if(opt_services) Array.prototype.push.apply(this.services_, opt_services);
...
if(typeof chrome != 'undefined')
	ServiceFinder.api = chrome.socket || chrome.experimental && chrome.experimental.socket;
...
ServiceFinder.prototype.ips = function(opt_service) {
  if(opt_service && !this.byService_[opt_service]) return [];
...
ServiceFinder.prototype.recv_ = function(sock, info) {
  ...
  packet.each('an', 12, function(rec) {
	...
	console.log(info.address, rec.asName());
	console.log("Incoming answer record", info.address, rec);
...
ServiceFinder.prototype.broadcast_ = function(sock, address) {
  ...
  // Answers
  this.services_.forEach(function(service){
	  //packet.push('an', new DNSRecord("", 12, 1, 10, service));
  });
